 ...
 * @echo  string
 */
add_filter( 'body_class', 'emav_add_body_pagetargeting_classes' );
function emav_add_body_pagetargeting_classes(  $classes  ) {
    $pagetargeting_option = get_option( '_emav_pagetargeting_option' );
    if ( $pagetargeting_option ) {
        // Add page targeting option to body class
        $classes[] = 'emav-targeting-' . esc_attr( $pagetargeting_option['option'] );
...
function emav_get_verify_form() {
    ...
    //	$confirm_btn_style     = 'style="background-color:#16641B;"';
    //	$not_confirm_btn_style = 'style="background-color:#A42828;"';
    $confirm_btn_style = '';
    $not_confirm_btn_style = '';
...
function emav_premium_features() {
    $features = array(
        __( 'Birthdate Verification', 'easy-marijuana-age-verify' ) => __( 'Verify visitors by birth date', 'easy-marijuana-age-verify' ),
        __( 'Box Design', 'easy-marijuana-age-verify' )             => __( 'Display content in a styled box', 'easy-marijuana-age-verify' ),
        __( 'Gradient Colors', 'easy-marijuana-age-verify' )        => __( 'Set gradient colors for background and box', 'easy-marijuana-age-verify' ),
