...
final class Easy_Marijuana_Age_Verify_Admin {
    ...
    public function register_settings() {
        ...
        add_settings_field(
            '_emav_pagetargeting_option',
            __( 'Page Targeting', 'easy-marijuana-age-verify' ) . ' <span class="dashicons dashicons-info emavoptionshovertip" title="' . __( 'Include or exclude a certain page.', 'easy-marijuana-age-verify' ) . '"></span>',
            'emav_settings_callback_pagetargeting_option_field',
            'easy-marijuana-age-verify',
            'emav_settings_general'
        );
        register_setting( 'easy-marijuana-age-verify', '_emav_pagetargeting_option', 'emav_pagetargeting_option_sanitize' );
        // AJAX Section
        add_settings_field(
            '_emav_settings_ajax',
        ...
        register_setting( 'easy-marijuana-age-verify', '_emav_disclaimer', 'wp_kses_post' );
    ...
    public function enqueue_scripts( $page ) {
        ...
        if ( function_exists( 'emav_get_verify_option_default_texts' ) ) {
            $verify_option_defaults = array();
            for ($option = 1; $option <= 7; $option++) {
                $verify_option_defaults[(string) $option] = emav_get_verify_option_default_texts( $option );
            }
            wp_localize_script( 'emav-admin-scripts', 'emavAdminSettings', array(
                'resetDefaultsConfirm' => __( 'Changing Verify For will reset the default text fields for this option. Continue?', 'easy-marijuana-age-verify' ),
                'verifyOptionDefaults' => $verify_option_defaults,
            ) );
        }
    }
